...
public class VertexTypeImpl extends VertexImpl implements VertexType{
	...
	public VertexTypeImpl(Node node) {
		super(node);
	...
	private static Node getNodeByName(DatabaseService db, String name){
		...
		if(typeSubRef.hasRelationship(relType, Direction.OUTGOING)){
			return typeSubRef.getSingleRelationship(relType, Direction.OUTGOING).getEndNode();
		}else{
			return null;
		}
	}
	...
	private static Node getOrCreateTypeSubRef(DatabaseService db){
		...
		if(refNode.hasRelationship(relType, Direction.OUTGOING)){
			return refNode.getSingleRelationship(relType, Direction.OUTGOING).getEndNode();
		}else{
	...
	public static VertexTypeImpl getOrCreateInstance(DatabaseService db, String name) {
		return new VertexTypeImpl(getOrCreateByDescriptor(new TypeNodeDescriptor(db, name, getImplementationClass())));
	}
